...
| **module batching, as it was BUILT** | measured negative four times (-26.6% R61b, -35.7% R96, -13.0% R105, regression R123) for two mechanical reasons, and BOTH are now gone. (1) Producing a batched body cost one full `mini_method_compile` per member; R141 made bodies relocatable and R147's `mono_wasm_jit_rebatch` re-frames from the retained bodies, so it is a memcpy. (2) The planner planned a plateau ONCE, on a quiescence this workload never reaches — reach 14% of the tier. Do not re-run the OLD arms or re-tune the OLD knobs (`batch_max`/`batch_bytes` measured non-binding). **R147 replaced the trigger and the reach is no longer the problem: 70.8% of registered methods co-located, 6,364 sites turned into `call <funcidx>`, in one boot, with no planner at all.** What IS the problem is that re-framing published methods crashes 6/6 (`function signature mismatch` at `wasm_jit_ethunk_cb`); `MONO_WASM_JIT_COLOCATE_DEPS` therefore ships default 0. The call form is exonerated (`COLOCATE_LOCAL_CALLS=0` still crashes), so bisect inside re-frame-and-rebind. Ceiling to know before spending on it: 55.0% of real call sites target the main module and can never be co-located (R129), only 22.8% of execution-weighted callee mass is under V8's 500-byte inline cap (R121b), and a callee is only inlined if the CALLER itself crosses V8's ~37,000-return tiering budget (R120). |
