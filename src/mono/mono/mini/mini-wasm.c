 ...
 * f-slot in wasm, skipping the resolve_fslot C helper). DEFAULT OFF — it is UNSAFE on threaded builds:
 * its "table[fslot] != null => installed" liveness check is wrong because jiterpreter prefills every
 * JitCall table slot with a NON-null placeholder (mono_jiterp_placeholder_jit_call, sig (i32,i32,i32,i32)
 * ->void). On a worker that cached an f-slot via the shared IC but hasn't instantiated that module
 * locally, the slot is the placeholder, the check passes, and call_indirect with the vcall functype traps
 * "function signature mismatch" (jit138). The C-helper path (mono_wasm_jit_vcall_resolve_fslot) syncs the
 * thread before returning an f-slot, so it's safe; the inline path only shortcut the ~7% jitted-override
 * vcalls anyway. =1 re-enables it (only safe on single-threaded builds, where un-installed slots are null). */
...
mono_wasm_jit_sync_thread (void)
{
	...
	while (synced < wj_reg_n) {
		...
		if (mono_wasm_jit_slot_live (wj_reg [synced].e)) { synced++; continue; }
...
mono_wasm_emit_method (MonoCompile *cfg)
{
	...
	if (mono_wasm_jit_ldaddr) {
		for (bb = cfg->bb_entry; bb; bb = bb->next_bb) {
			MONO_BB_FOR_EACH_INS (bb, ins) {
				...
				if (lvt == 0 || lvt == WASM_VOID || wasm_valtype_group (lvt) < 0) { fail = "ldaddr of non-scalar local"; fail_op = OP_LDADDR; goto done; }
...
#endif /* HOST_BROWSER (OP_LDADDR addressable-locals pass) */
	...
	lc.addrslot = (naddrbytes > 0) ? addrslot : NULL;
	...
	lc.storeguard = 0; lc.check_ti = -1;
#ifdef HOST_BROWSER
	{
		extern int mono_wasm_jit_storeguard;
		if (mono_wasm_jit_storeguard && nextra < 32) {
			...
			lc.storeguard = 1; uses_calls = TRUE;
		}
	}
#endif
#ifdef HOST_BROWSER
			...
			for (bb = cfg->bb_entry; bb; bb = bb->next_bb) {
				MONO_BB_FOR_EACH_INS (bb, ins) {
					...
					if (d < 0 || d >= nvreg || isref [d])
						continue;
					/* Byref / managed interior pointer (STACK_MP): route to the conservatively-pinning ref
					 * shadow stack too, so the heap object it points into is pinned across GC points — a byref
					 * left in a GC-invisible wasm local dangles if its target moves. Covers ldflda
					 * (&heapObj.field), Span<T> internals, ref locals, etc. Over-marking is safe: a byref into
					 * native/stack memory (or the OP_LDADDR frame) pins nothing. */
					if (ins->type == STACK_MP) { isref [d] = TRUE; changed = TRUE; continue; }
					switch (ins->opcode) {
					...
					case OP_LOAD_MEMBASE:
						...
						break;
					case OP_GET_EX_OBJ:   /* caught exception object (JITted catch handler, EH enabled): a managed ref */
						isref [d] = TRUE; changed = TRUE;
						break;
...
#define LDIV_RAISE(EXC_ID) do { \
		...
		if (mono_wasm_jit_cppeh) { wasm_op (&body, WASM_OP_UNREACHABLE); } \
		else { switch (ret_vt) { \
			...
			default: break; } \
		EMIT_REF_LEAVE (); wasm_op (&body, WASM_OP_RETURN); } } \
	} while (0)
	...
	for (bb = cfg->bb_entry; bb; bb = bb->next_bb, ++i) {
		...
		MONO_BB_FOR_EACH_INS (bb, ins) {
			...
			case OP_LDIV: case OP_LDIV_UN: case OP_LREM: case OP_LREM_UN: {
				...
				if (ldiv_rti < 0) { if (nextra >= 32) { fail = "too many callee types"; goto done; } extra_types [nextra] = rt; ldiv_rti = 2 + nextra++; }
				uses_calls = TRUE;
...
#undef STOREMI
			...
			case OP_CALL: case OP_VOIDCALL: case OP_FCALL: case OP_LCALL: case OP_RCALL: {
				...
				if (call_fslot > 0) {
					 ...
					 * direct path (no interp residual). emitted: ensure_fslot(call_method, call_fslot). */
					{
						...
						if (eti < 0) { if (nextra >= 32) { fail = "too many callee types"; goto done; } extra_types [nextra] = et; eti = 2 + nextra++; }
					 ...
					 * throwing AOT callee would escape uncaught until Build 2 adds wasm-EH — test non-throwing. */
					{
						...
						if (aot_ok && mono_wasm_jit_aot_call_target (call_method, &aot_addr, &aot_rgctx, &aot_has_extra)) {
							...
							if (mono_wasm_jit_cppeh) {
								...
								{ int *wargs = (int *) call->call_info; for (ai = 0; ai < (int) ct.nparams; ++ai) if (!wasm_ld (&body, &lc, wargs [ai])) { fail = "call arg ld"; goto done; } }
								if (aot_has_extra) wasm_i32_const (&body, (gint32) (intptr_t) aot_rgctx);   /* rgctx (ftndesc.arg) — only if the body has it */
								wasm_i32_const (&body, (gint32) (intptr_t) aot_addr);    /* AOT target table index (cinfo->addr) */
								wasm_op (&body, WASM_OP_CALL_INDIRECT); wasm_uleb (&body, (guint32) nti); wasm_uleb (&body, 0);
								if (ct.ret != WASM_VOID) {   /* store ret inside the try */
									if (ct.ret == WASM_I32) wasm_emit_subword_ret_norm (&body, csig->ret);   /* raw AOT body: dirty upper bits */
									if (!wasm_st (&body, &lc, ins->dreg)) { fail = "call dreg"; goto done; }
								}
							...
							{ extern int mono_wasm_jit_verbose; static int n_il = 0; if (mono_wasm_jit_verbose >= 3 && n_il++ < 64) { char *cn = mono_method_get_full_name (call->method); printf ("WASM_JIT_INLINE_AOT[%d] %s -> %s\n", n_il, mname, cn); g_free (cn); } }
							break;
						}
					}
					 ...
					 * imethod already transformed and only does the GC-free marshal. (i32)->void. */
					{
						...
						if (tpi < 0) { if (nextra >= 32) { fail = "too many callee types"; goto done; } extra_types [nextra] = tp; tpi = 2 + nextra++; }
						wasm_i32_const (&body, (gint32) (intptr_t) call_method);
			...
			case OP_CALL_REG: case OP_VOIDCALL_REG: case OP_FCALL_REG: case OP_LCALL_REG: case OP_RCALL_REG:
				 ...
				 * other shapes bail to interp. */
					if (is_membase && call->method && (call->method->flags & METHOD_ATTRIBUTE_VIRTUAL)) {
						 ...
						 * cached may be absent on THIS thread. BUG: that liveness check is WRONG on threaded
						 * builds — jiterpreter prefills JitCall slots with a NON-null placeholder, so the check
						 * passes and call_indirect traps "function signature mismatch" (jit138). Gated OFF by
						 * default (MONO_WASM_JIT_VCALL_INLINE_IC); the C-helper slow path below syncs the thread. */
						...
						wasm_op (&body, WASM_OP_ELSE);
							 ...
							 * into the (shared, unchanged) residual below; if AOT we call + br past the residual. */
							{
								...
								if (mono_wasm_jit_vcall_aot) {
									...
									WasmFuncType at, at_ne, aott, eht; int ati = -1, ati_ne = -1, aotti = -1, ehti = -1;
									...
									if (ati_ne < 0) { if (nextra >= 32) { fail = "too many callee types"; goto done; } extra_types [nextra] = at_ne; ati_ne = 2 + nextra++; }
									uses_calls = TRUE;
	...
	if (eh_on) {
		...
		if (nrefslots > 0) {
			wasm_op_local (&body, WASM_OP_LOCAL_GET, (guint32) refbase_idx);
			wasm_i32_const (&body, (gint32) (nrefslots * 4));   /* sizeof (MonoObject*) == 4 on wasm32 */
		...
		if (naddrbytes > 0) {
			wasm_op_local (&body, WASM_OP_LOCAL_GET, (guint32) addrbase_idx);
			wasm_i32_const (&body, (gint32) naddrbytes);
			wasm_op (&body, WASM_OP_I32_ADD);
			wasm_i32_const (&body, (gint32) (intptr_t) mono_wasm_jit_addr_leave);
			wasm_op (&body, WASM_OP_CALL_INDIRECT); wasm_uleb (&body, (guint32) leave_ti); wasm_uleb (&body, 0);
		}
