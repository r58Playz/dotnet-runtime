...
struct InterpMethod {
	...
	guint8 wasm_jit_entry_fast_ok; // runtime wasm JIT (MONO_WASM_JIT_AOT_ENTRY): this method has been through the jiterp native->interp entry at least once with wasm_jit_slot > 0 AND mono_wasm_jit_admit satisfied, so later entries may take the fast path that skips the InterpFrame/LMF/maybe_compile/admit scaffolding. Admission is terminal (memoized DFS, state 2), so caching it is sound.
